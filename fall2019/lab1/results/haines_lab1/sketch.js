...
Multiple canvases
Added mouseClicked function 
included plot for selected ball's changes in size and color
    ...
    for (let i = this.id + 1; i < balls_to_draw; i++) {
      ...
      if (distance < minDist) {
        ...
        if (this.vx > 0.1 && this.vy > 0.1) {
          ...
          if (this.others[i].green < 0) {
            this.others[i].green = 0;
            this.others[i].blue = 0;
          }
          // Reset ball size
