...
function preload() {
  stateCoordinates = loadTable("data/statecoordsScaled2.csv", "csv", "header");
  tableCrimes = loadTable("data/table.csv", "csv", "header");

  loadStateID();
  //linegraph
  stats = loadTable('table.csv','csv','header');
...
function drawLegend() {
  ...
  fill(82,82,82);
  textSize(25);
  text(titleText + newYear, 150, 0);

  // Sets the legend
  fill(255);
  strokeWeight(.5);
  rect(600, 150, 140, 245);
  fill(82,82,82);
  textSize(11);
  titleText = getTitle(false);
  text(titleText, 605, 175);
  
  if (currentCrimeType == 0) {
    // legend 1 - 25,000 - 15,000
    drawDataRangeLabel(15000, 25000, label1x, label1y) ;

    // legend 2  15,000 - 10,000
    drawDataRangeLabel(10000, 15000, label2x, label2y) ;

    // legend 3 - 10,000 - 5,000
    drawDataRangeLabel(5000, 1000, label3x, label3y) ;

    // legend 4 - 5,000 - 0
    drawDataRangeLabel(1, 5000, label4x, label4y) ;

    // legend 5 - No data 
    drawDataRangeLabel(0, 5000, label5x, label5y) ;
    
  } else if (currentCrimeType == 1) {
    drawDataRangeLabel(pct4 * violentMax, violentMax, label1x, label1y);
    drawDataRangeLabel(pct3 * violentMax, pct4 * violentMax, label2x, label2y);
    drawDataRangeLabel(pct2 * violentMax, pct3 * violentMax, label3x, label3y);
    drawDataRangeLabel(pct1 * violentMax, pct2 * violentMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * violentMax, label5x, label5y);
  } else if (currentCrimeType == 2) {
    drawDataRangeLabel(pct4 * murderMax, murderMax, label1x, label1y);
    drawDataRangeLabel(pct3 * murderMax, pct4 * murderMax, label2x, label2y);
    drawDataRangeLabel(pct2 * murderMax, pct3 * murderMax, label3x, label3y);
    drawDataRangeLabel(pct1 * murderMax, pct2 * murderMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * murderMax, label5x, label5y);
  } else if (currentCrimeType == 3) {
    drawDataRangeLabel(pct4 * rapeMax, rapeMax, label1x, label1y);
    drawDataRangeLabel(pct3 * rapeMax, pct4 * rapeMax, label2x, label2y);
    drawDataRangeLabel(pct2 * rapeMax, pct3 * rapeMax, label3x, label3y);
    drawDataRangeLabel(pct1 * rapeMax, pct2 * rapeMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * rapeMax, label5x, label5y);
  } else if (currentCrimeType == 4) {
    drawDataRangeLabel(pct4 * robberyMax, robberyMax, label1x, label1y);
    drawDataRangeLabel(pct3 * robberyMax, pct4 * robberyMax, label2x, label2y);
    drawDataRangeLabel(pct2 * robberyMax, pct3 * robberyMax, label3x, label3y);
    drawDataRangeLabel(pct1 * robberyMax, pct2 * robberyMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * robberyMax, label5x, label5y);
  } else if (currentCrimeType == 5) {
    drawDataRangeLabel(pct4 * assaultMax, assaultMax, label1x, label1y);
    drawDataRangeLabel(pct3 * assaultMax, pct4 * assaultMax, label2x, label2y);
    drawDataRangeLabel(pct2 * assaultMax, pct3 * assaultMax, label3x, label3y);
    drawDataRangeLabel(pct1 * assaultMax, pct2 * assaultMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * assaultMax, label5x, label5y);
  } else if (currentCrimeType == 6) {
    drawDataRangeLabel(pct4 * propertyMax, propertyMax, label1x, label1y);
    drawDataRangeLabel(pct3 * propertyMax, pct4 * propertyMax, label2x, label2y);
    drawDataRangeLabel(pct2 * propertyMax, pct3 * propertyMax, label3x, label3y);
    drawDataRangeLabel(pct1 * propertyMax, pct2 * propertyMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * propertyMax, label5x, label5y);
  } else if (currentCrimeType == 7) {
    drawDataRangeLabel(pct4 * burglaryMax, burglaryMax, label1x, label1y);
    drawDataRangeLabel(pct3 * burglaryMax, pct4 * burglaryMax, label2x, label2y);
    drawDataRangeLabel(pct2 * burglaryMax, pct3 * burglaryMax, label3x, label3y);
    drawDataRangeLabel(pct1 * burglaryMax, pct2 * burglaryMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * burglaryMax, label5x, label5y);
  } else if (currentCrimeType == 8) {
    drawDataRangeLabel(pct4 * larcenyMax, larcenyMax, label1x, label1y);
    drawDataRangeLabel(pct3 * larcenyMax, pct4 * larcenyMax, label2x, label2y);
    drawDataRangeLabel(pct2 * larcenyMax, pct3 * larcenyMax, label3x, label3y);
    drawDataRangeLabel(pct1 * larcenyMax, pct2 * larcenyMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * larcenyMax, label5x, label5y);
  } else if (currentCrimeType == 9) {
    drawDataRangeLabel(pct4 * vehicleMax, vehicleMax, label1x, label1y);
    drawDataRangeLabel(pct3 * vehicleMax, pct4 * vehicleMax, label2x, label2y);
    drawDataRangeLabel(pct2 * vehicleMax, pct3 * vehicleMax, label3x, label3y);
    drawDataRangeLabel(pct1 * vehicleMax, pct2 * vehicleMax, label4x, label4y);
    drawDataRangeLabel(0, pct2 * vehicleMax, label5x, label5y);
  }
}
...
function mousePressed() {
  ...
  if (mouseButton == LEFT) {
    ...
    textSize(10);
    textAlign(CENTER);
    text(state, topLeftX, topLeftY + 10, 175, 275);
    textAlign(LEFT);

    if (newTotalint > 0) {
      text("Violent Crime rate - " + violent, topLeftX + indentation, topLeftY + 25, 175, 275);
      text("Murder and nonnegligent", topLeftX + indentation, topLeftY + 50, 175, 275);
      text("      manslaughter rate - " + murder, topLeftX + indentation, topLeftY + 60, 175, 275);
      text("Forcible rape rate - " + rape, topLeftX + indentation, topLeftY + 85, 175, 275);
      text("Robbery rate - " + robbery, topLeftX + indentation, topLeftY + 110, 175, 275);
      text("Aggravated assault rate - " + assault, topLeftX + indentation, topLeftY + 135, 175, 275);
      text("Property crime rate - " + property, topLeftX + indentation, topLeftY + 160, 175, 275);
      text("Burglary rate - " + burglary, topLeftX + indentation, topLeftY + 185, 175, 275);
      text("Larceny-theft rate - " + larceny, topLeftX + indentation, topLeftY + 210, 175, 275);
      text("Motor vehicle theft rate - " + vehicle, topLeftX + indentation, topLeftY + 235, 175, 275);
      text("TOTAL - " + newTotalint, topLeftX + indentation, topLeftY + 260, 175, 275);
    } else {
      text("No data found", topLeftX + indentation, topLeftY + 25, 175, 275);
    }
  }
}
